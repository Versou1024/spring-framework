...
public class ProxyConfig implements Serializable {
	/*
	 * ProxyConfig 代理配置
	 * 用于创建代理proxy需要使用的配置的便利超类，以确保所有代理创建者具有一致的属性。
	 * 包括以下属性：frozen、exposeProxy、optimize、proxyTargetClass
	 */
	...
	private boolean proxyTargetClass = false; // 代理目标类，为false，就使用JDK代理，否则就是用Cglib代理

	// 标记是否对代理进行优化。true：那么在生成代理对象之后，如果对代理配置进行了修改，已经创建的代理对象也不会获取修改之后的代理配置。
	// 如果exposeProxy设置为true，即使optimize为true也会被忽略。
	private boolean optimize = false; // optimize 默认是false，开启后，可以使用Cglib代理进行优化
	...
	boolean opaque = false; // 设置是否应阻止此配置创建的proxy被强制转换为“adviced”。默认值为“false”，这意味着任何AOP代理都可以被转换为adviced。

	//标记代理对象是否应该被aop框架通过AopContext以ThreadLocal的形式暴露出去。
	//当一个代理对象需要调用它【自己】的另外一个代理方法时，这个属性将非常有用。默认是是false，以避免不必要的拦截。
	boolean exposeProxy = false; // 是否暴露proxy对象

	// 标记是否需要冻结代理对象，即在代理对象生成之后，是否允许对其进行修改，默认为false.
	// 当我们不希望调用方修改转换成Advised对象之后的代理对象时，就可以设置为true 给冻结上即可
	private boolean frozen = false; // 配置冻结
	...
	public void setProxyTargetClass(boolean proxyTargetClass) {
		...
		// 如果目标类是一个接口，那么将为给定接口创建一个JDK代理。
		// 如果该目标类是任何其他类，则将为给定类创建CGLIB代理。
		// 注意：根据具体代理工厂的配置，如果未指定接口（且未激活接口自动检测），也将应用代理目标类行为。
		// 从 DefaultAopProxyFactory#createAopProxy 代码可知的
	...
	public void setOptimize(boolean optimize) {
		// 例如，优化optimize通常意味着advise的更改在创建代理后不会生效。
		// 因此，默认情况下禁用优化。如果其他设置排除了优化，则“true”的优化值可能会被忽略：
		// 例如，如果“exposeProxy”设置为“true”，并且与优化不兼容。
