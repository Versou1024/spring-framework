	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		...
		try {
			...
			if (!this.equalsDefined && AopUtils.isEqualsMethod(method)) {
				...
				// 是否为hashCode方法，且没有被代理
				return hashCode();
			}
			// 下面两段做了很有意思的处理：DecoratingProxy和Advised接口的方法  都是是最终调用了config，也就是this.advised去执行的~~~~
			...
			// 获取当前被代理的method上的过滤器链 interception chain
			// 这里的拦截器就是MethodInterceptor，其中会到方法进行通知增强
			...
			if (chain.isEmpty()) {
				...
				// 然后使用AopUTils进行引用：targetmethod、argsToUes
				// 直接对method进行反射调用，无须增强
				retVal = AopUtils.invokeJoinpointUsingReflection(target, method, argsToUse);
			}
			else {
				// We need to create a method invocation...
				// 有MethodInterceptor过滤器链，就去创建ReflectiveMethodInvocation并执行
				// 创建一个invocation ，此处为ReflectiveMethodInvocation  最终是通过它，去执行前置加强、后置加强等等逻辑
				MethodInvocation invocation = new ReflectiveMethodInvocation(proxy, target, method, args, targetClass, chain);
				// Proceed to the joinpoint through the interceptor chain.
				// 进行MethodInterceptor的增强处理
			...
			if (target != null && !targetSource.isStatic()) {
				// Must have come from TargetSource. 一定是来自TargetSource。
				// 对于 SingletonTargetSource.isStatic 返回true，必须要额外处理
				targetSource.releaseTarget(target);
			}
			if (setProxyContext) {
				// Restore old proxy.
				// 前面要求暴露Proxy，将Proxy设置到ThreadLocal上，这里执行完后，就需要转换为之前的oldProxy
