...
public interface TransactionDefinition {
	...
	// 事务的基本定义信息： 隔离级别、传播特性、超时时间、是否只读、事务名称
	...
	// 下文提到的加入当前事务，指的是底层使用同一个Connection，但是事务状态对象是可以重新创建的
	// ，并不影响。文章提到的当前只存在一个事务，表示的是共用底层的一个Connection，而不在乎创建了多少个事务状态对象(TransactionStatus)。
